...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        <a href="{{ route('purchase-orders.index') }}" class="text-slate-400 hover:text-slate-600 transition-colors">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-slate-800">New Purchase Order</h1>
            <p class="text-slate-500 text-sm mt-0.5">Order products from a supplier</p>
        </div>
    </div>

    <form method="POST" action="{{ route('purchase-orders.store') }}" id="po-form" class="space-y-6">
        @csrf
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <a href="{{ route('suppliers.create') }}" class="text-indigo-600 text-xs hover:underline mt-1 inline-block">+ Add new supplier</a>
                </div>
...
function updateVariants(sel, idx) {
    ...
    if (product) {
        product.variants.forEach(v => {
            varSel.innerHTML += `<option value="${v.id}">${v.name} (${v.unit_label})</option>`;
        });
        if (product.variants.length === 1) varSel.value = product.variants[0].id;
    }
}
